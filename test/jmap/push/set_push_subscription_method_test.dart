...
void main() {
  group('test to json set pushSubscription method', () {
    ...
    test('set pushSubscription method and response parsing', () async {
      ...
      final setPushSubscriptionInvocation = requestBuilder.invocation(setPushSubscriptionMethod);
      final response = await (requestBuilder
          ..usings(setPushSubscriptionMethod.requiredCapabilities))
        .build()
        .execute();

      final setPushSubscriptionResponse = response.parse<SetPushSubscriptionResponse>(
        setPushSubscriptionInvocation.methodCallId,
        SetPushSubscriptionResponse.deserialize);

      expect(setPushSubscriptionResponse!.created![Id('dab246')]!.id, equals(expectedCreated.id));
    });
  });
}