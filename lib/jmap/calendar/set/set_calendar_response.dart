...
class SetCalendarResponse extends SetResponse<Calendar> {
  SetCalendarResponse(AccountId accountId,
      {State? newState,
      State? oldState,
      Map<Id, Calendar>? created,
      Map<Id, Calendar?>? updated,
      Set<Id>? destroyed,
  ...
  static SetCalendarResponse deserialize(Map<String, dynamic> json) {
    return SetCalendarResponse(
      ...
      created: (json['created'] as Map<String, dynamic>?)?.map((key, value) =>
          MapEntry(const IdConverter().fromJson(key),
              Calendar.fromJson(value as Map<String, dynamic>))),
