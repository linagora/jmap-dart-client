...
class SetAddressBookResponse extends SetResponse<AddressBook> {
  SetAddressBookResponse(
    AccountId accountId, {
    State? newState,
    State? oldState,
    Map<Id, AddressBook>? created,
    Map<Id, AddressBook?>? updated,
    Set<Id>? destroyed,
  ...
  static SetAddressBookResponse deserialize(Map<String, dynamic> json) {
    return SetAddressBookResponse(
      ...
      created: (json['created'] as Map<String, dynamic>?)?.map(
        (key, value) => MapEntry(
          ...
          AddressBook.fromJson(value as Map<String, dynamic>),
        ),
      ),
